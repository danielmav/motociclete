...
- `database/newsletter_brevo.php <input.json> > out.yml` (PHP 8.1 Laragon) generează **YAML-ul editorului Brevo** (Developer mode → Ctrl+A, paste) din fragmentele `documente/newsletter/parts/*.yml` (gitignored; tăiate din `template.txt`: `head/news/mid1/model×2/mid2/product_row×2/tail`, placeholdere `{{KEY}}`). Input JSON = știre (titlu/imagine/link/paragrafe) + **2 modele** (URL/slug din catalogul local: nume, preț EUR, cover, descriere) + **6 produse BikerShop** (URL/id → LIVE `productsByIds`: nume, preț RON brut, imagine); orice câmp e suprascriibil (`input.example.json`). Nu parsează YAML — lucrează pe text (escapare `'`→`''`, UUID-uri noi pe blocurile duplicate); verificare = diff structural cu template-ul (`id/content/src/href` mascate). BikerShop-ul cere IP-ul de dev whitelisted (altfel avertisment + doar câmpurile din input).
