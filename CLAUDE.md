...
- **Buton „Programează drive test" pe pagina produs** = modal de lead (email la dealer, NU link la `/drive-test/`, deci neafectat de `TESTRIDE_ENABLED`); apare doar când `testride_allowed` (calculat în `CatalogController::renderProduct`: CFMOTO = toate, Yamaha doar `top_slug` ∈ {`motociclete`,`scutere`}); la ATV/marine „Cere ofertă" devine butonul principal.
