...
- **BikerShop** `bikershop_ps9` @ `5.254.67.10` (user `bikershop_test` — are de fapt **GRANT ALL**; „read-only" e convenția runtime-ului portalului, scrieri de mentenanță permise prin PDO din `tmp_*.php`, dar NU pe produse/prețuri — vezi supplierpricing). PrestaShop 9 / MariaDB 10.11, **~334.801 produse**, prefix `ps_`, multistore (filtrează `id_shop=1` ȘI `id_lang=1` pe `product_lang`). Behind Cloudflare → conectare doar la IP origine (3306 nu trece prin Cloudflare); dev IP whitelisted în Remote MySQL.
  - **SSH pe serverul bikershop:** alias `ssh bikershop` (`5.254.67.10:1187`, user `bikershop`, cheie `id_ed25519_bikershop`); PrestaShop în `~/public_html`. Cloudflare challenge blochează curl ȘI Chrome headless pe bikershop.ro → verifică paginile live din browser sau cu `curl` de pe server prin SSH.
  - **Pagini CMS:** `ps_cms_lang` are 4 rânduri per pagină (shop 1,2 × lang 1,2) — actualizează-le pe toate. Paginile legale rescrise (aug 2026, GDPR/OUG 34/2014/OUG 140/2021); surse + backup în `documente/bikershop-legal/`.
  - **Scripturi CLI pe serverul bikershop (PrestaShop 9.0.2):** bootstrap = `define('_PS_ADMIN_DIR_', '<root>/__admin322y'); require '<root>/config/config.inc.php';` (tipar `modules/advrider_related/cli/sync.php`), binar dovedit `/usr/local/bin/ea-php84` (8.4, gd+curl+pdo_mysql; `ea-php81` e tot 8.4 acolo). Scripturile noastre stau în `~/tools/<nume>/` (afară din docroot; `_advrider/` = ale agenției). Root PS = `~/public_html`, imagini produs `img/p/a/b/c/…/<id_image>.jpg` + `-<tip>.jpg` (PS_IMAGE_FORMAT=jpg; webp-urile existente sunt vechi). Cache: Redis (`teamwant_redis`, hook-urile ObjectModel îl invalidează) + LiteSpeed (`litespeedcache`, purge DOAR prin header HTTP → imposibil din CLI, TTL 24h). Hook-urile `actionProductUpdate` trag supplierpricing/leopartsfilter/sendinblue etc. → pentru scrieri punctuale preferă SQL direct + invalidare Redis țintită, nu `Product::update()`.
  - **Catalog Yamaha hyperdrive (accesorii complete, GUID `bf96ad91-…`):** ~2135 SKU; per produs `description` (RO), `productNameEn`, per variantă `internalDescription` (EN), `images[]` (CDN 2000×2000, conține și PDF-uri de montaj → filtrează pe extensie), `prices[0].amount` EUR. Dă sporadic 5xx/gol la paginare → retry cu backoff. Snapshotul local `yamaha_catalog` (RefAudit/dup473) se învechește → `fetch_yamaha_catalog.php --apply` înainte de audituri.
  - Categoria 473 are `link_rewrite` = `piese-si-componente-yamaha` (nu „accesorii-oem"); schimbarea categoriei implicite mută URL-ul produsului, PS face redirect canonic automat. Cronul supplierpricing (`supplierpricing_runner.sh`) rulează la 30 min → o modificare pusă în `ps_supplierpricing_queue` (`id_product,last_update,in_progress,force_update`) se recalculează în max. 30 min.
  - **Tema Warehouse (iqitthemeeditor):** fontul body = cheia `iqitthemeed_typo_bfont_g_url` în `ps_configuration` (2 rânduri: global + shop 1). Diacriticele urâte s-au reparat cu `&subset=latin-ext` la URL-ul Google Fonts; după schimbări de config golește cache-ul din BO.
...
- **Buton „Programează drive test" pe pagina produs** = modal de lead (email la dealer, NU link la `/drive-test/`); din sept. 2026 e gated ȘI pe `TESTRIDE_ENABLED` (`{% if testride_allowed and testride_enabled %}` în `product.twig`; modalul `test-ride` nici nu se mai randează în `partials/lead-modals.twig`) → **ascuns peste tot cât timp flagul e `0`**; când flagul revine pe `1`, apare doar unde `testride_allowed` (calculat în `CatalogController::renderProduct`: CFMOTO = toate, Yamaha doar `top_slug` ∈ {`motociclete`,`scutere`}); la ATV/marine „Cere ofertă" devine butonul principal.
...
- **Slug duplicat la salvare produs:** `products` are `UNIQUE(brand, slug)` → `ProductController::save` verifică întâi cu `Repository::productBySlug()` (plus `catch` pe `23000` ca plasă de siguranță) și, la conflict, NU salvează nimic: pune formularul trimis în `$_SESSION['product_draft']` (consumat de `form()`) și afișează un flash cu produsul care ocupă slug-ul + link la el. Cazul tipic = model reimportat de la Yamaha (importerul dă slug fără an, ex. `tricity-300`) peste unul **scos din ofertă** (`is_active=0`, invizibil în catalog). Înainte, INSERT-ul crăpa cu 500 și operatorul pierdea tot formularul.
...
- **Bara header e ÎNTUNECATĂ** (`#0E0E10`, ca footer-ul) — override în `app-v2.css` pe `.site-header`/`.headerv2` (text/iconițe/burger albe, accent roșu la hover/`[aria-expanded]`). Restul sitului e alb → **elementele noi în header se stilizează pe culori deschise**. Excepții care rămân pe alb: dropdown-ul mega (`.megav2` = `var(--bg)`) și overlay-ul de căutare (`.search` = `#fff`) → orice `.iconbtn` din ele are nevoie de o regulă care readuce `color:var(--ink)` (vezi `.headerv2 .search .iconbtn`).
...
- **Home „Accesorii din BikerShop"** (`HomeController::featuredAccessories`): `Client::featuredProducts()` alege produse random în 2 pași ieftini (felie de coperți din `ps_image` de la o ancoră random pe PK → detalii pe `id IN (...)` + `active=1`, max 2 produse/ancoră); rezultatul e **file-cached** în `storage/cache/home_accessories.cache` (TTL 6h; golul nu se cache-uiește). ⚠️ Vechea variantă („primul produs activ cu copertă de la un id random în sus") scana ~100k rânduri/produs → TTFB 3–10 s pe home (sept. 2026). Container: `cache_dir` = `storage/cache`.
...
- **Numele culorii:** `product_images.caption` (doar tip `color`) → afișat pe pagina produs sub imaginea principală (`[data-color-name]`, actualizat la click pe thumb) + `title`/`aria-label` pe thumbs. Editabil în admin (input pe tile-ul de culoare, `color_caption[]` paralel cu `color[]` — `replaceImages` le împerechează pe index). Importul Yamaha pune `colourName`-ul oficial din hyperdrive (culorile se aleg din **toate** variantele, nu doar `variants[0]` — la R9 fiecare variantă are doar culoarea ei; captions trec prin `downloadImages` ca `image_captions`). Backfill istoric din numele fișierelor: `database/backfill_color_names.php --apply` (best-effort, umple doar caption gol; rulat local+prod, ~112 fișiere fără pattern rămân goale → se completează din admin).
...
## Newsletter Brevo (generator YAML)

- `App\Newsletter\Generator` generează **YAML-ul editorului Brevo** (Developer mode → Ctrl+A, paste) din fragmentele `storage/newsletter/parts/*.yml` (**gitignored** — conțin id-uri/URL-uri din contul Brevo; tăiate din template-ul exportat `documente/newsletter/template.txt`: `head/news/mid1/model×2/mid2/product_row×2/tail`, placeholdere `{{KEY}}`; se copiază pe server cu `scp`). Input = știre (titlu/imagine/link/paragrafe) + **2 modele** (URL/slug din catalogul local: nume, preț EUR, cover, descriere; slug fără an → cel mai nou `slug-YYYY`) + **6 produse BikerShop** (URL/id → LIVE `productsByIds`: nume, preț RON brut, imagine); orice câmp e suprascriibil (`documente/newsletter/input.example.json`). Nu parsează YAML — lucrează pe text (escapare `'`→`''`, UUID-uri noi pe blocurile duplicate); verificare = diff structural cu template-ul (`id/content/src/href` mascate).
- **Admin** `{base}/newsletter` (`Admin\NewsletterController` + `templates/admin/newsletter/index.twig`): formular (upload imagine în `/media/newsletter/`, context upload `newsletter`) → YAML în textarea + „Copiază"; ultimul formular rămâne în sesiune; nu scrie în DB. **CLI:** `database/newsletter_brevo.php input.json > out.yml` (PHP 8.1 Laragon). BikerShop cere IP-ul whitelisted (altfel avertisment + doar câmpurile din input).
...
- ⚠️ **Referința PrestaShop = SKU-ul Yamaha COMPLET fără cratime (12 caractere)**: `BR8-HIPER-KT-10` → `BR8HIPERKT10`. Logica veche tăia ultimele 2 caractere (presupuse „cod de mărime") → coduri ambigue care se ciocnesc cu alte produse (`BR8HIPERKT00` e cu totul alt produs). `Importer::normalizeSku()` e sursa canonică; forma trunchiată rămâne doar rezervă la match. Sufixul NU e mereu `00`.
- **Catalog Yamaha COMPLET** (≠ cel per model): subtree `bf96ad91-485c-4c2b-86a0-c1857d86097b`, fără filtrul `products:<PID>` → ~1683 produse / ~1930 SKU-uri, toate de 12 caractere. `database/fetch_yamaha_catalog.php --apply` → tabela `yamaha_catalog` (`schema_yamaha_catalog.sql`, rulată din `migrate_admin.php`).
...
- **Îmbogățire accesorii pe BikerShop (imagini + descriere)** — `database/bikershop/enrich_yamaha_accessories.php`, rulează **PE SERVERUL BIKERSHOP** (bootstrap PrestaShop 9 din `config/config.inc.php`, ca `modules/advrider_related/cli/sync.php`): copie la `/home2/bikershop/tools/yamaha-enrich/` (scp după modificări), cron `0 4 1,15 * *` cu `/usr/local/bin/ea-php84`, log `~/logs/yamaha_enrich_cron.log` + `tools/yamaha-enrich/logs/` (enrich-*.log, rollback-*.sql, prices-*.csv). Cauza problemei: produsele adăugate din diagrame (cat. **2545**, ~350k rânduri) au cod+preț dar 0 imagini/descriere → apar goale în tabul „Piese & accesorii" al portalului. Scriptul ia catalogul Yamaha complet (hyperdrive, ~2100 SKU) și, pentru produsele ACTIVE cu `reference` = SKU Yamaha: adaugă imaginile CDN (2000×2000) prin `Image::add()` + **redimensionare GD locală** (`local_resize`, toate `ImageType` + formatele din `PS_IMAGE_FORMAT`; doar dacă produsul are 0 imagini) — ⚠️ NU `ImageManager::resize`: hook-ul `actionOnImageResizeAfter` e tratat de `teamwant_redis` fără id_product → curățare oarbă Redis, ~4 s per apel (27 s/imagine vs. 0,8 s), completează `description` goală (RO din `description`, EN din `internalDescription`), îl pune în cat. **473** ca implicită. Prețurile = raport CSV (EUR×5,30 vs. rrp din `ps_product_supplier`); cu `--fix-prices` corectează DOAR produsele active cu exact un furnizor, prin fluxul supplierpricing (rescrie special|rrp, price=brut/1,21, coadă `force_update`) — NU e în cron, se rulează la cerere (rulat 2026-09-23: 158 produse). Restul (furnizori divergenți etc.) rămân în CSV pt. decizie manuală. Dry-run implicit; `--apply`, `--only=ID|SKU`, `--limit=N`. Hyperdrive dă uneori 5xx la rafale → `http_get` reîncearcă 3× cu backoff. Cache: Redis invalidat direct prin `teamwant_redis->hookActionObjectProductUpdateAfter`; LiteSpeed NU poate fi purjat din CLI (purge = header HTTP) → paginile pot rămâne vechi până la TTL (24h).
...
- Import: `database/import_yamaha_accessories.php --apply` (toate modelele / cron pe **1 și 15** ale lunii, 06:00). Admin: la save cu PID **nou/schimbat** → sync automat (protejat, nu strică salvarea) + buton „Resincronizează" (`ProductController::syncAccessories`). Reîmprospătare modele vechi = **cron** pe server. Setare PID în bloc: `tests/set_yamaha_pids.php` din CSV.

## Audit referințe/prețuri accesorii BikerShop (cat. 473)

- Categoria **473** = piese/accesorii Yamaha curatate (imagini + descrieri); **2545** = produse adăugate de un script separat, fără imagini/descriere, dar cu cod complet și preț.
- Preț accesorii Yamaha pe bikershop: **brut (cu TVA) = EUR × 5,30**, net = brut / 1,21. Cursul se derivă și se afișează în raport (mediana), nu se presupune.
- `src/Accessories/RefAudit.php` (READ-ONLY) + raport `tests/dup_ref_473.php` (guard `APP_ENV=dev` sau `?token=` din `TOOLS_TOKEN`; bifele salvează `storage/dup473_selection.json`) → `database/apply_dup_ref_473.php` (dry-run implicit, `--apply`, `--only=<id>`; rollback **per rulare** în `storage/dup473_rollback_<ts>.sql` — unul cumulativ ar anula la re-aplicare și corecțiile vechi). `tests/build_price_selection.php` generează selecția pentru toate prețurile divergente cu exact un furnizor.
...
- **Email → DB:** `Support\Mailer::send($to,$subj,$body,$context)` persistă **fiecare** email în `email_log`
  ...
  **Layout HTML emailuri** = `Support\EmailTemplate`: `wrap()` (header `#0E0E10` + `assets/img/email-logo.png` 320px + accent roșu + footer contact
  ...
  Test real de pe prod: `tmp_*.php` (autoload + `.env` + `Mailer` cu `setBrand(...)`), scp-uit în docroot, `ea-php81 tmp.php adresa@`, apoi șters — trimite la adresa userului, NU la info@/showroom@. Footer-ul cu departamente vine din `contact_departments` (`$container['content']->departments()`).
...
- ⚠️ **Prețurile și starea activ/inactiv de pe BikerShop sunt guvernate de modulul `supplierpricing`** (sursa în `documente/supplierpricing/`, gitignored). NU scrie direct în `ps_product.price` / `ps_product_shop.price` / `active` — cronul le suprascrie la următoarea rulare.
  - Sursa reală a prețului = `ps_product_supplier.product_supplier_reference`, cu formatul **`cantitate|special_price|rrp_price`** (sau 5 câmpuri, cu perioadă de promoție `d.m.Y`). **Primul câmp e STOCUL**, nu un discount — cronul îl scrie în `StockAvailable`. `special > rrp` invalidează produsul (stoc 0, prețuri −1).
  - Cronul calculează `ps_product.price = rrp_price / SUPPLIERPRICING_VAT_RATE` (=1.21) — vezi `controllers/front/cron.php:546-551`.
  - Modificarea corectă = rescrii câmpurile 2 și 3 (păstrând stocul), apoi pui produsul în `ps_supplierpricing_queue` (`force_update=1`) și lași cronul să recalculeze. A pune în coadă FĂRĂ a schimba sursa = pierzi modificarea.
  - **Dezactivarea manuală nu ține**: un produs inactiv cu stoc 0 și brand permis e re-activat (`cron.php`, „Caz 2"), dacă ajunge în coadă. Ca să rămână dezactivat, șterge rândurile din `ps_product_supplier` — regula `SUPPLIERPRICING_DISABLE_NO_SUPPLIER_PRODUCTS=1` are prioritate în fața re-activării. (Alternativ: categoria în `SUPPLIERPRICING_OUTOFSTOCK_CATEGORIES`, dar scoate toată categoria de sub gestiunea automată.)
  - Prioritatea între furnizori e per-combinație: `SPECIALPRICE_SUPPLIER` (=4) câștigă chiar la stoc 0, apoi `OVERRIDE_SUPPLIER_ORDER` (=1) dacă are stoc, apoi oricare cu stoc > 0. La produsele cu **mai mulți furnizori nu ghici** prețul — rezolvă manual.
- ⚠️ **Nu face JOIN pe `UPPER(reference)`** (sau orice funcție pe coloană) peste `ps_product`: anulează indexul `reference_idx` și devine scan pătratic pe ~335k produse — a încetinit situl live până la 500. Colația e oricum case-insensitive, deci `reference IN (...)` găsește și variantele cu litere mici. ⚠️ În PHP însă cheile de array SUNT case-sensitive → normalizează cu `strtoupper()` când indexezi rezultatele, altfel pierzi potriviri tăcut.
  Timeout-ul clientului `mysql.exe` **NU** oprește interogarea pe server (rămâne să ardă CPU) — verifică `information_schema.PROCESSLIST` și oprește cu `KILL <id>`, dar doar userul care a lansat-o (sau unul cu privilegiul `PROCESS`) o poate opri.
...
**Deploy prin SSH** (alias `~/.ssh/config` `dualmotors` → `109.163.231.49:1187`, user `dualmotors`, cheie `id_ed25519_dualmotors`): `ssh dualmotors 'cd /home/dualmotors/public_html/motociclete.com.ro && git pull --ff-only origin main'` — Claude poate face pull-ul singur după push (nu mai e nevoie de „Update from Remote" din cPanel). **Un hook `PostToolUse` rulează automat acest pull după fiecare `git push` reușit pe acest proiect** — dar NU e garantat (a lipsit când push-ul era înlănțuit cu `&&`/`|grep`): după push verifică `git log --oneline -1` pe server și fă pull manual dacă a rămas în urmă. Warning-ul post-quantum pe stderr e inofensiv. Working tree-ul de pe server trebuie curat (`composer.phar` untracked e ok; `.htaccess` e skip-worktree). La dependențe noi pull-ul NU rulează composer (vezi mai jos).
...
- **Cron-uri (ea-php81, `/usr/local/bin/ea-php81`):** accesorii Yamaha pe 1 și 15 ale lunii 06:00 (`import_yamaha_accessories.php --apply`; perechea lui pe serverul bikershop = `enrich_yamaha_accessories.php`, 04:00 aceleași zile); curs valutar zilnic 07:00 (`update_currency.php`); retention GDPR zilnic 03:30 (`retention.php --apply` — anonimizare IP 30z / PII 12 luni, ștergere `email_log`/`client_otp`); loguri în `/home/dualmotors/`.
...
- **drivetest** — sistem „drive test" (PHP procedural + MySQLi, DB `dualmotors_testdrive`, fără Composer;
  ...
  **Butoanele „Programează drive test" sunt ascunse site-wide** (cerere client, aug 2026) prin flagul `TESTRIDE_ENABLED`
  (`.env`, default `0`; global Twig `testride_enabled` în hero/bandă home, mega-meniu, footer, **și butonul + modalul de lead de pe pagina produs** — sept. 2026, vezi secțiunea Catalog). Endpointul `POST /api/lead/test-ride` rămâne activ (fără UI care să-l cheme).
  ...
  - Schema reală ≠ `schema.sql` (lipsesc `rezervare_tokens`, `sesiuni.ora_sfarsit_duminica/pauza_masa_*/maps_url/vizibila_de`,
    `clienti.cnp/ci_*`). **Auth API admin** = `includes/auth.php`: `admin/index.php` (servit doar după Basic Auth) marchează sesiunea `dt_admin` + emite token; `api/*` cere sesiunea + header `X-Admin-Token` (wrapper peste `fetch` în admin) pentru `admin_*`, `modele.php`, `token.php`, `all_modele.php`. Public rămân doar `sesiuni_active/sesiune_details/modele_sesiune/sloturi/rezervare_create` + `pdf_generate.php`. **Email:** `includes/mail.php` — `sendMail()` folosește **PHPMailer-ul portalului** (`../vendor/autoload.php`, SMTP localhost:25, quoted-printable; fallback `mail()` QP); ⚠️ `mail()` brut cu HTML pe o linie lungă (>998 car.) pleca „sent" dar NU ajungea în Gmail. From `website@`, `MAIL_SHOWROOM=showroom@`, departamente hardcodate `EMAIL_DEPARTMENTS`; `rezervare_create` notifică showroom-ul (`notifyShowroomRezervare`, după commit, eșecul nu strică rezervarea); clientul primește confirmare (`notifyClientRezervare`, cu contact Vânzări moto 0722354437 / showroom@) + PV-ul după semnare (`pdf_generate.php`).
